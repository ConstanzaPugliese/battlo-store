...
let main = document.getElementById(`cardsKylie`)
let title = document.createElement(`h1`)
title.setAttribute(`class`, `col-lg-12 col-xs-12 my-4`)
title.textContent = `Kylie Minibag`
main.prepend(title)
// P R O D U C T O S //
const fromJson = localStorage.getItem(`products`)
const Products = JSON.parse(fromJson)
let ProductsCategory = []
let currentCategory = `Kylie`
// C A R R I T O //
let shoppingCart = [];
let total = 0;
const DOMtext = document.getElementById(`textKylie`);
const DOMshoppingCart = document.getElementById(`shoppingCartKylie`);
const DOMbuttonEmpty = document.getElementById(`buttonEmptyKylie`);
const DOMtotal = document.getElementById(`totalKylie`);
const DOMinstallmentValue = document.getElementById(`installmentValueKylie`);
const DOMbuttonBuy = document.getElementById(`buttonBuyKylie`);
...
function printProducts() {
    filterCategory()
    let cards = document.getElementById(`cardsKylie`)
...
function block () {
    const div1 = document.getElementById(`noneBlockKylie`)
    div1.style.display = `block`;
}
function none () {
    const div2 = document.getElementById(`noneBlockKylie`)
