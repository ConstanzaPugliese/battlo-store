...
let main = document.getElementById(`cardsStraps`)
let title = document.createElement(`h1`)
title.setAttribute(`class`, `col-lg-12 col-xs-12 my-4`)
title.textContent = `Correas`
main.prepend(title)
// P R O D U C T O S //
const fromJson = localStorage.getItem(`products`)
const Products = JSON.parse(fromJson)
let ProductsCategory = []
let currentCategory = `straps`
// C A R R I T O //
let shoppingCart = [];
let total = 0;
const DOMtext = document.getElementById(`textStraps`);
const DOMshoppingCart = document.getElementById(`shoppingCartStraps`);
const DOMbuttonEmpty = document.getElementById(`buttonEmptyStraps`);
const DOMtotal = document.getElementById(`totalStraps`);
const DOMinstallmentValue = document.getElementById(`installmentValueStraps`);
const DOMbuttonBuy = document.getElementById(`buttonBuyStraps`);
...
function printProducts() {
    filterCategory()
    let cards = document.getElementById(`cardsStraps`)
...
function block () {
    const div1 = document.getElementById(`noneBlockStraps`)
    div1.style.display = `block`;
}
function none () {
    const div2 = document.getElementById(`noneBlockStraps`)
