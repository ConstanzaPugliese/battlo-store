...
let main = document.getElementById(`cardsKendall`)
let title = document.createElement(`h1`)
title.setAttribute(`class`, `col-lg-12 col-xs-12 my-4`)
title.textContent = `Kendall Minibag`
main.prepend(title)
// P R O D U C T O S //
const fromJson = localStorage.getItem(`products`)
const Products = JSON.parse(fromJson)
let ProductsCategory = []
let currentCategory = `Kendall`
// C A R R I T O //
let shoppingCart = [];
let total = 0;
const DOMtext = document.getElementById(`textKendall`);
const DOMshoppingCart = document.getElementById(`shoppingCartKendall`);
const DOMbuttonEmpty = document.getElementById(`buttonEmptyKendall`);
const DOMtotal = document.getElementById(`totalKendall`);
const DOMinstallmentValue = document.getElementById(`installmentValueKendall`);
const DOMbuttonBuy = document.getElementById(`buttonBuyKendall`);
...
function printProducts() {
    filterCategory()
    let cards = document.getElementById(`cardsKendall`)
...
function block () {
    const div1 = document.getElementById(`noneBlockKendall`)
    div1.style.display = `block`;
}
function none () {
    const div2 = document.getElementById(`noneBlockKendall`)
